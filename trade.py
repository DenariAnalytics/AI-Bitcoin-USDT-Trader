...
def fetch_and_preprocess_data(trading_pair, interval):
    ...
    data.drop(columns=['Timestamp'], inplace=True)  # Remove the 'Timestamp' column
    return data
...
while True:
    try:
        ...
        trading_signal = setup.calculate_trading_signals(y_pred[-1], data['close'].values[-1])
        execute_trade(trading_signal)
        sleep(interval * 60)  # Sleep for the duration of the interval (in seconds)
